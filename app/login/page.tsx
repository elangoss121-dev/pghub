...
import { Building2, Eye, EyeOff, ArrowRight, Mail, Lock, AlertCircle } from "lucide-react"
...
export default function LoginPage() {
  ...
  const handleRoleChange = (newRole: "resident" | "admin") => {
    ...
    setError("")
    if (newRole === "admin") {
      setEmail("sivakumarelango10@gmail.com")
      setPassword("Manju1303")
    } else {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      setError("Invalid email, password, or account status. Please try again.")
      setIsLoading(false)
    }
  }
    ...
    <div className="min-h-screen bg-background flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-sidebar flex-col justify-between p-12 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-3xl font-bold text-white mb-4 text-balance">Welcome back to your smart PG dashboard</h2>
          <p className="text-sidebar-foreground/60 leading-relaxed mb-8">Manage payments, food, laundry, and more — all from one place.</p>
          <div className="grid grid-cols-2 gap-4">
            {[
              { value: "500+", label: "PGs" },
              { value: "25K+", label: "Residents" },
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="w-full max-w-md">
          ...
          <div className="flex bg-muted rounded-xl p-1 mb-6">
            {(["resident", "admin"] as const).map((r) => (
              <button key={r} type="button" onClick={() => handleRoleChange(r)}
                className={cn("flex-1 py-2.5 rounded-lg text-sm font-medium transition-all capitalize", role === r ? "bg-card shadow-sm text-foreground" : "text-muted-foreground hover:text-foreground")}>
          ...
          {error && (
            <motion.div 
              ...
              className="flex items-center gap-2 p-3 mb-4 rounded-xl bg-destructive/10 border border-destructive/20 text-destructive text-sm"
            >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-background border border-border rounded-xl pl-9 pr-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring transition-shadow" 
                />
              </div>
            </div>
            <div>
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-background border border-border rounded-xl pl-9 pr-10 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-ring transition-shadow" 
                />
          ...
          <div className="mt-6 p-4 bg-muted/50 rounded-xl border border-border">
            <p className="text-xs text-muted-foreground text-center">
              ...
              <span className="text-muted-foreground">
                {role === "admin" 
                  ? "Admin: sivakumarelango10@gmail.com / Manju1303"
                  : "Resident: rahul@email.com / password123"
                }
              </span>
            </p>
          </div>
